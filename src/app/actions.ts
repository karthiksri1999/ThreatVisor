...
import {
  suggestThreatsAndMitigations,
  ThreatSuggestionsOutput,
} from "@/ai/flows/threat-suggestions";
...
export async function analyzeThreatsAction(
  ...
  const methodology =
    (formData.get("methodology") as string) || prevState.analyzedMethodology;
  ...
  try {
    ...
    if (!threats || !threats.threats) {
      return {
        threats: null,
        error:
          "The analysis returned an empty or invalid response. Please try again or adjust your input.",
        components: null,
        analyzedDsl: dsl,
    ...
    if (e && typeof e.message === "string") {
      ...
      if (e.message.includes("API key not valid")) {
        errorMessage =
          "Your Google AI API key is missing or invalid. Please add a valid GOOGLE_API_KEY to your .env file and restart the server. Refer to the README.md for instructions.";
      } else if (
        e.message.includes("503") ||
        e.message.toLowerCase().includes("model is overloaded")
      ) {
        errorMessage =
          "The analysis service is temporarily busy and could not process the request. This can happen during peak hours. Please try again in a few moments.";
      } else {
